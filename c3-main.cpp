...
int main(int arg_count, char* arg_vector[]){  
    ...
    if(arg_count ==1){
      NDT_selected=1; // if no input is inserted, NDT will be selected as default.
    }
    if(arg_count == 2 && strcmp(arg_vector[1], "NDT")==0){
      NDT_selected= 1;
         }
    if(arg_count == 2 && strcmp(arg_vector[1], "ICP")==0){
      NDT_selected= 0;
              }
	...
	while (!viewer->wasStopped())
  	{
		...
		if(!new_scan){      
            if(scans_count == 0) {
			//Using GroudTruth at the beginning of localization. Using lidar data afterward for localize
			...
			pcl::VoxelGrid<PointT> vg; //Declare Voxel grid
			vg.setInputCloud(scanCloud); //Set input cloud
			double filterRes = 1.0; //Declare filter resolution
			vg.setLeafSize(filterRes, filterRes, filterRes); //Set leaf size for voxel grid
			typename pcl::PointCloud<PointT>::Ptr cloudFiltered (new pcl::PointCloud<PointT>); //Declare cloud filtered in the Heap
			vg.filter(*cloudFiltered); //Apply filter on cloud points by reference


			//Find pose transform by using ICP or NDT matching:

			//Use NDT or ICP algorithm (based on user input), and create transform_match
			Eigen::Matrix4d transform_match = NDT_selected ? NDT(mapCloud, cloudFiltered, pose): ICP(mapCloud, cloudFiltered, pose);          
            
			//TODO: Find pose transform by using ICP or NDT matching
			pose = getPose(transform_match);

			//TODO: Transform scan so it aligns with ego's actual pose and render that scan
          	PointCloudT::Ptr transformed_scan (new PointCloudT);
			pcl::transformPointCloud (*cloudFiltered, *transformed_scan, transform_match);
...
Eigen::Matrix4d ICP(PointCloudT::Ptr target, PointCloudT::Ptr source, Pose startingPose) 
{
    ...
    //ICP parameters:  
    icp.setTransformationEpsilon(.001);
    int iterations = 60;
    ...
    cout << "ICP converged: " << icp.hasConverged () << "  with score: " << icp.getFitnessScore () <<  " time: " << time.toc() <<  " ms" << endl;
...
Eigen::Matrix4d NDT(PointCloudT::Ptr mapCloud, PointCloudT::Ptr source, Pose startingPose) 
{
    ...
    cout << "NDT converged: " << ndt.hasConverged () << " with score: " << ndt.getFitnessScore () <<  " time: " << time.toc() <<  " ms" << endl;
